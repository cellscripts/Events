...
$('#set-event-position, #find-event').click(function() {
...
$('#map-page').live(
   ...
   function()
   {
      ...
      // his event position else he want to retrieve the position
      if (requestType == 'set')
      {
         navigator.geolocation.getCurrentPosition(
            function(location)
            {
               ...
               navigator.notification.alert(
                  'The Event position has been saved',
                  function(){},
                  'Info'
               );
            },
            ...
            {
               navigator.notification.alert(
                  'Unable to retrieve event position. Is your GPS enabled?',
                  function(){
      ...
      else
      {
         ...
         else
         {
            navigator.geolocation.watchPosition(
               ...
               function(error)
               {
                  console.log("Unable to retrieve the event location: " + error.message);
               },
               geolocationOptions
            );
         }
...
function createPositionsHistoryList(idElement, positions)
{
   ...
   for(var i = 0; i < positions.length; i++)
   {
      ...
      .click(
         ...
         {
            var position = new Position();
            var oldLength = position.getPositions().length;
            var $parentUl = $(this).closest('ul');

            position.deletePosition($(this).closest('li').index());
            if (oldLength == position.getPositions().length + 1)
            {
